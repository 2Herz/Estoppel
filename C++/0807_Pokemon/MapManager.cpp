...
{
	ImgClass* map = new ImgClass;
	map->LoadImg(_hWnd, "Images/PewterCity.bmp");
...
void MapManager::SetMap(int _mapNumber)
{
	m_currentMap = HOME_TOWN;
}
...
{
	m_mapList[m_currentMap]->Render(_hdc,m_mapPosition.x - m_camPosition.x, m_mapPosition.y - m_camPosition.y);
}
