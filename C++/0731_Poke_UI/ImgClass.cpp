...
void ImgClass::Render(HDC _destDC, int _posX, int _posY, int _width, int _height)
{
	...
	if (m_isTrans == TRUE)
	{
		if (m_opacity == 255)
		{
			GdiTransparentBlt(_destDC, _posX, _posY, _width, _height, _destDC, 0, 0, m_size.cx, m_size.cy, m_colorkey);
		}
		else
		{
			//	1. 빈 종이 만들기
			...
			//	3. 빈 종이에 Transp....
			GdiTransparentBlt(empty.GetImgDC(), 0, 0, _width, _height, _destDC, 0, 0, m_size.cx, m_size.cy, m_colorkey);
