...
void NPCManager::SaveNPC()
{
	...
	*/
	temp[0] = { 0,0,3,0,1,2,{ 2,2 },{ 5,2 } };
	temp[1] = { 1,0,0,3,4,0,{ 5,5 },{ 4,4 } };
	temp[2] = { 2,0,0,2,2,0,{ 7,2 },{ 3,3 } };
	temp[3] = { 3,0,0,1,4,0,{ 9,5 },{ 2,2 } };
	temp[4] = { 4,0,3,1,4,4,{ 9,7 },{ 2,2 } };
	...
	if (fout.fail() == false)
	{
		...
		for (int i = 0; i < NPC_TOTAL_NUMBER; i++)
		{
			NPC* npc = new NPC(data[i].m_number,
				...
				data[i].m_Position,
				data[i].m_moveBox);
			m_npcList.insert(std::make_pair(data[i].m_number, npc));
		}
	}
...
void NPCManager::Update(float _elapseTime)
{
	if (m_stopMode)
	{
		auto find = m_npcList.find(m_activeNPC);
		find->second->DoAction(_elapseTime);
	}
	else
	{
...
void NPCManager::Render(HDC _hdc)
{
	for (auto i = m_npcList.begin(); i != m_npcList.end(); i++)
	{
		...
		if (i->second->GetState() == NPC_STATE::ENCOUNTER)
		{
