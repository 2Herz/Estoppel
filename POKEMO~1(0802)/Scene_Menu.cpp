...
bool Scene_Menu::WndProc(HWND _hWnd, UINT _message, WPARAM _wParam, LPARAM _lParam)
{
	...
	case WM_KEYDOWN:
	{
		...
		case 'Z':
		{
			switch (m_Box.m_Select)
			{
			case 0: {SCENE_MGR.SetScene(SCENE_DIC); }break;
			case 1: {SCENE_MGR.SetScene(SCENE_POKEMON); }break;
			case 2: {SCENE_MGR.SetScene(SCENE_BAG); }break;
			case 3: {SCENE_MGR.SetScene(SCENE_TRAINER); }break;
			case 4: {SCENE_MGR.SetScene(SCENE_REPORT); }break;
			case 5: {SCENE_MGR.SetScene(SCENE_SETTING); }break;
			case 6: {SCENE_MGR.SetScene(SCENE_MAIN); }break;	//	닫기
			}
		}break;

		case 'x':
		case 'X': {SCENE_MGR.SetScene(SCENE_MAIN); }break;

		case VK_RETURN: {SCENE_MGR.SetScene(SCENE_MAIN); }break;
